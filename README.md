...
Register an account and create a room with a challenge prompt
Share the room code with friends
Friends join from a separate browser/incognito tab
Host starts a round
Participants submit their creative concept
AI generates a full output for each submission in real time
Host scores each submission (0–10) and can eliminate participants
Start the next round or finish the battle


Environment Variables
Backend (backend/.env)
VariableDescriptionSECRET_KEYJWT signing key — change this in productionDATABASE_URLSQLite file pathOLLAMA_URLOllama server URL (default: http://localhost:11434)OLLAMA_MODELModel to use (default: llama3.2:1b)
Frontend (frontend/.env.local)
VariableDescriptionNEXT_PUBLIC_API_URLBackend URL (default: http://localhost:8000)
